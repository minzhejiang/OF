...
void ofApp::setup(){
    ...
    for (curAgent; curAgent < MAX_AGENTS; curAgent++) {  // only enable if we disable mouse gravity
      Agents[curAgent].start(ofRandom(ofGetWidth()),ofRandom(ofGetHeight()/2, ofGetHeight()));
    }
}
...
void ofApp::mouseMoved(int x, int y ){
  for (int i=0; i < MAX_AGENTS; i++) {
    Agents[i].calcGravityWell(x,y);//between mouse location and the vecotr
  }
}
...
void ofApp::mousePressed(int x, int y, int button){
    ...
    curAgent++;
    if (curAgent >= MAX_AGENTS) {  // make sure it does not go over, kill zero and give it new information
      curAgent = 0;
    }
}
